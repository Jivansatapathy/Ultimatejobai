...
import { getInvestorRoleBySlug, INVESTOR_ROLES } from "@/data/startupBoardRoles";
import { searchSeniorJobs } from "@/services/seniorJobService";
import { Briefcase, MapPin, Building2, ChevronRight, DollarSign, TrendingUp, ExternalLink, ArrowRight } from "lucide-react";
...
export default function InvestorRolePage() {
  const { role: slug = "" } = useParams<{ role: string }>();
  const role = getInvestorRoleBySlug(slug);
  const [jobs, setJobs] = useState<Job[]>([]);
  const [jobCount, setJobCount] = useState<number | null>(null);
  const [loading, setLoading] = useState(true);

  if (!role) return <Navigate to="/investors" replace />;
    ...
    "@graph": [
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 2, "name": "Investor Roles", "item": "https://ultimatejobai.vercel.app/investors" },
          { "@type": "ListItem", "position": 3, "name": role.title, "item": `https://ultimatejobai.vercel.app/investors/${slug}` },
        ],
      },
      {
        "@type": "CollectionPage",
        ...
        "url": `https://ultimatejobai.vercel.app/investors/${slug}`,
      },
    ],
  };

  const relatedRoles = role.related.map(s => INVESTOR_ROLES.find(r => r.slug === s)).filter(Boolean) as typeof INVESTOR_ROLES;

  return (
    <>
      <title>{role.title} Jobs — VC & Private Equity Roles | Hizorex</title>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-2">
        <div className="mx-auto max-w-6xl flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <Link to="/investors" className="hover:text-emerald-700 transition-colors">Investor Roles</Link>
          <ChevronRight className="h-3 w-3" />
      ...
      {/* Hero */}
      <section className="py-12 px-4 border-b bg-emerald-50 border-emerald-100">
        <div className="mx-auto max-w-6xl">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="h-4 w-4 text-emerald-700" />
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-700">VC / PE Role</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex items-center gap-2 bg-emerald-700 text-white font-bold px-6 py-2.5 rounded-full hover:bg-emerald-800 transition-colors"
            >
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            ...
            {loading ? (
              ...
              <p className="text-gray-400 text-sm">No live postings right now — check back daily or search all investor roles.</p>
            ) : (
              <div className="space-y-3">
                {jobs.map((job, i) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-start justify-between gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3.5 hover:border-emerald-300 hover:shadow-sm transition-all group"
                  >
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-900 text-sm leading-tight group-hover:text-emerald-700 transition-colors">{job.title}</p>
                    ...
                    <ExternalLink className="h-3.5 w-3.5 text-gray-300 group-hover:text-emerald-500 shrink-0 mt-0.5" />
                  </a>
                ))}
              </div>
            )}
            {(jobCount ?? 0) > 6 && (
              <Link to={`/find-jobs?q=${encodeURIComponent(role.searchQ)}`} className="inline-flex items-center gap-1 mt-4 text-sm font-semibold text-emerald-700 hover:text-emerald-900 transition-colors">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {role.skills.map(s => (
                <span key={s} className="text-xs font-semibold px-2.5 py-1 rounded-full border border-emerald-200 bg-emerald-50 text-emerald-700">{s}</span>
              ))}
            </div>
          </div>
              ...
              <div className="space-y-2">
                {relatedRoles.map(r => (
                  <Link
                    key={r.slug}
                    to={`/investors/${r.slug}`}
                    className="flex items-center justify-between text-sm text-gray-700 hover:text-emerald-700 transition-colors group"
                  >
                    {r.title}
                    <ChevronRight className="h-3.5 w-3.5 text-gray-300 group-hover:text-emerald-500" />
                  </Link>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-2xl bg-emerald-700 text-white p-5">
            <h3 className="font-bold text-base mb-2">Find Your Next Investor Role</h3>
            ...
            <Link
              to="/auth?mode=signup"
              className="block text-center bg-white text-emerald-800 font-bold px-4 py-2 rounded-full text-sm hover:bg-emerald-50 transition-colors"
            >
              Start Free
            </Link>
          </div>
