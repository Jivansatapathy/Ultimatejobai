...
export default function Settings() {
  ...
  const handleSaveMail = async () => {
    ...
    try {
      ...
      await autoApplyService.saveCredential(trimmedEmail, sanitized);
  ...
  const handleUpdateProfile = async () => {
    ...
    setIsSaving(true);
    try {
      await careerService.updateProfile(profile);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="lg:col-span-1 space-y-2">
            <nav className="flex flex-col gap-1">
              <button className="flex items-center gap-3 px-4 py-3 rounded-xl bg-teal-500/10 border border-teal-500/20 text-teal-400 font-bold text-sm text-left">
                <User className="h-4 w-4" />
                Personal Info
              </button>
              <button className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:bg-white/[0.05] hover:text-white font-medium text-sm text-left transition-colors">
          ...
          <main className="lg:col-span-2 space-y-8">
            ...
            <section className="rounded-3xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-md p-6 md:p-8">
              ...
              <div className="space-y-6">
                ...
                {appPassword !== "" && (
                  <p className="text-[10px] text-amber-400 font-medium mt-2 flex items-center gap-1">
                    <Info className="h-3 w-3" /> Save your new App Password before running the test.
                  </p>
                )}
              </div>
            </div>
            ...
            {profile && (
              <section className="rounded-3xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-md p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="w-full rounded-2xl border border-white/10 bg-white/[0.05] text-slate-100 px-4 py-3.5 text-sm font-medium outline-none focus:ring-2 focus:ring-teal-500/40 focus:border-teal-500/40 focus:bg-white/10 transition-all appearance-none cursor-pointer"
                    >
                ...
                <Button
                  ...
                  disabled={isSaving}
                  variant="outline"
                  className="w-full mt-8 h-12 rounded-2xl border-white/10 bg-white/[0.03] text-slate-300 font-bold hover:bg-white/10 hover:text-white"
                >
