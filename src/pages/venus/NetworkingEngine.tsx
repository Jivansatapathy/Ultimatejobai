    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-800 leading-relaxed">{rec.description}</p>
          {hasUrl ? (
            ...
            <span className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-gray-700">
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-700 mt-1">AI-powered introductions, communities & executive moves.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12 gap-2 text-gray-700">
            ...
            <>
              ...
              {others.length > 0 && (
                <div className="space-y-3">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-700">Other Opportunities</p>
        ...
        <div>
          <div className="rounded-2xl border border-gray-200 bg-white p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-gray-800">Executive Moves</h3>
            </div>
            {movesLoading ? (
              ...
              <p className="text-xs text-gray-700 text-center py-4">No recent exec moves found.</p>
            ) : (
              <div className="space-y-3">
                {moves.map((m, i) => {
                  ...
                  return (
                    <Wrapper key={i} {...(wrapperProps as object)}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {validUrl && <ExternalLink className="h-3 w-3 shrink-0 text-gray-700 group-hover:text-blue-600 transition-colors mt-0.5" />}
                      </div>
                      <p className="text-[10px] text-gray-700 mt-1">{m.source} · {new Date(m.published_at).toLocaleDateString()}</p>
                    </Wrapper>
                  );
                })}
              </div>
        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="h-9 rounded-xl border border-gray-300 bg-gray-100 px-3 pr-8 text-xs text-gray-800 outline-none focus:border-blue-500 transition-colors appearance-none cursor-pointer"
            >
              ...
              <div key={ev.id || i}
                ...
                <div>
                  <div className="flex items-center justify-between mb-2 gap-2">
                    ...
                    <span className="text-[10px] text-gray-700 flex items-center gap-1 shrink-0">
                      <CalendarDays className="h-3 w-3" />
                      {ev.date_text || "Date TBA"}
                    </span>
                  </div>
                  <p className="text-sm font-bold text-gray-900 leading-snug mb-1.5">{ev.title}</p>
                  <p className="text-xs text-gray-700 flex items-start gap-1.5 mb-3">
                ...
                {ev.link ? (
                  ...
                  <span className="text-xs font-bold text-gray-700">No link available</span>
                )}
              </div>
            ))}
          </div>
